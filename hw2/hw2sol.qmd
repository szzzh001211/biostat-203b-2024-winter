...
```{r,eval=TRUE}
filtered_labevents_csv$charttime <- as.POSIXct(filtered_labevents_csv$charttime, format = "%Y-%m-%d %H:%M:%S", tz ="UTC")
nrow(filtered_labevents_csv)
head(filtered_labevents_csv,10)
```
**Answer:** It takes 0.034 seconds for the ingest+select+filter process. There are 24855909 rows and they match those in Q2.3.
...
```{r,eval=TRUE}
filtered_labevents_parquet$charttime <- as.POSIXct(filtered_labevents_parquet$charttime, format = "%Y-%m-%d %H:%M:%S", tz ="UTC")
nrow(filtered_labevents_parquet)
head(filtered_labevents_parquet,10)
```
**Answer:** It takes 0.301 seconds for the ingest+select+filter process, which is almost half of the time of the csv file. There are 24855909 rows and they match those in Q2.3.
...
filtered_labevents_duckdb$charttime <- as.POSIXct(filtered_labevents_duckdb$charttime, format = "%Y-%m-%d %H:%M:%S", tz ="UTC")
nrow(filtered_labevents_duckdb)
head(filtered_labevents_duckdb,10)
```
**Answer:** It takes 0.696 seconds for the ingest+convert+select+filter process, which is almost same as the csv and slower than the parquet file. This may be because it has more commands than the Q2.4 and Q2.5. There are 24855909 rows and they match those in Q2.3.
