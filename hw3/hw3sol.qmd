...
```{bash}
#| eval: false
ln -s /Users/zihengzhang/Downloads/203B/203b-hw/hw2/labevents_parquet ./labevents_pq
```
...
lab_info <- arrow::open_dataset("./labevents_pq", format = "parquet") |>
  filter(subject_id == patient_id) |>
...
plot(Procedure_plot)
```
**Answer:** This patient has been admitted to the hospital three times. Each admission results in the top 3 diagnoses. Therefore, for the subtitle, we select one of the admission records and displayed the top 3 diagnoses for that admission. Here we select admission record with `hadm_id` = 21243435 and we only display the first few words of the long title because the long title is too long.\
For the label of procedure, since the long title is too long, we only display the words before the first comma of the long title. We manually set the shape of the procedure to be the total 9 procedures and then all of the procedures are displayed in the plot.
...
```{bash}
#| eval: false
ln -s /Users/zihengzhang/Downloads/203B/203b-hw/hw2/chartevents.parquet ./chartevents_pq
```
...
chartevents_info <- arrow::open_dataset("./chartevents_pq", format = "parquet") |>
  filter(subject_id == patient_id) |>
  filter(itemid %in% c(220045, 220180, 220179, 220210, 223761)) |>
...
icu_plot <- ggplot(chartevents_info, aes(x = charttime, y = valuenum, color = abbreviation)) +
  geom_point(size = 1) +
  geom_line() +
...
```{r,eval=TRUE}
head(labevents_tble, 10, width = Inf)
```
...
item_icu_subset <- read_csv("~/mimic/icu/d_items.csv.gz") |>
  ...
  mutate(label = tolower(label))
icustays_tble_subset <- select(icustays_tble, subject_id, stay_id, intime, outtime)
...
chartevents_tble <- arrow::open_dataset("./chartevents_pq", 
  ...
  arrange(subject_id, label, charttime) |>
  group_by(subject_id, label, stay_id) |>
  summarise(firstvalue = first(valuenum)) |>
  spread(label, firstvalue)
...
```{r,eval=TRUE}
head(chartevents_tble, 10, width = Inf)
```
...
```{r,eval=TRUE}
b <- ggplot(mimic_icu_cohort, aes(los, fill = insurance)) +
  geom_histogram()
  labs(title = "Length of ICU stay vs Insurance")
plot(b)
```
**Comment:** Overall, the length of ICU stay tends to be relatively short. As the length of stay increases, the frequency decreases gradually. The number of people using `other` insurances and `Medicare` insurance remains the same in almost all lengths of stay. In contrast, the number of people using `Medicaid` is very small, accounting for only about one-tenth of the total of the other two types combined.


```{r,eval=TRUE}
c <- ggplot(mimic_icu_cohort, aes(los, fill = marital_status)) +
  geom_histogram()
  labs(title = "Length of ICU stay vs Martial Status")
plot(c)
```
**Comment:** Overall, the length of ICU stay tends to be relatively short. As the length of stay increases, the frequency decreases gradually. The number of people who are `married` marital status remains the largest in almost all lengths of stay. In contrast, the number of people who are `divorced` and `widowed` marital status is small.

```{r,eval=TRUE}
d <- ggplot(mimic_icu_cohort, aes(los, fill = gender)) +
  geom_histogram()
  labs(title = "Length of ICU stay vs Gender")
plot(d)
```
**Comment:** Overall, the length of ICU stay tends to be relatively short. As the length of stay increases, the frequency decreases gradually. The number of `female` and `male` remains almost the same in all lengths of stay.

```{r,eval=TRUE}
e <- ggplot(mimic_icu_cohort, aes(x = age_at_intime, y = los)) +
  geom_point() +
  labs(title = "Length of ICU stay vs Age at intime")
plot(e)
```
**Comment:** The length of ICU stay tends to be relatively short for all ages. As the age increases, the length of stay tends to increase and the length of stay seems to be longer for people aged 65-80.
...
ggtitle(f, "Length of ICU stay vs Last available lab measurements before ICU stay")
plot(f)
```
**Comment:** `Bicarbonate` and `hematocrit` exhibit relatively uniform distributions of length of ICU stay. Meanwhile, distributions of `creatinine`, `glucose`, and `wbc` are relatively concentrated in regions where measurement values are smaller, and as these values increase, length of stay tends to decrease. The distributions of `Chloride`, `potassium`, and `sodium` are similar to normal distribution.
...
g2 <- ggplot(mimic_icu_cohort, aes(x = non_invasive_blood_pressure_systolic, y = los)) +
  geom_point(size = 0.5) +
  xlim(0, 300)

g3 <- ggplot(mimic_icu_cohort, aes(x = non_invasive_blood_pressure_diastolic, y = los)) +
  geom_point(size = 0.5) +
  xlim(0, 220)
...
g <- grid.arrange(g1, g2, g3, g4, g5, ncol = 1, nrow = 5)
ggtitle(g, "Length of ICU stay vs First vital measurements within the ICU stay")
plot(g)
```
**Comment:** The distributions of `heart_rate`, `non_invasive_blood_pressure_systolic`, `non_invasive_blood_pressure_diastolic`, and `respiratory_rate` are almost uniform. `temperature_fahrenheit` around 35 tends to correlate with slightly shorter lengths of stay. Most values fall within the range of 90 to 105 Fahrenheit. Within this range, as `temperature_fahrenheit` increases, the length of stay tends to initially increase before decreasing. The longest lengths of stay occur around a temperature of approximately 98 Fahrenheit, which is the normal body temperature.

```{r,eval=TRUE}
h <- ggplot(mimic_icu_cohort, aes(los, fill = first_careunit)) +
  geom_histogram()
  labs(title = "Length of ICU stay vs First ICU unit")
plot(h)
```
**Comment:** Overall, the length of ICU stay tends to be relatively short. As the length of stay increases, the frequency decreases gradually. `Neuro Intermediate`, `Neuro Stepdown`, and `Neuro SICU` have significantly shorter lengths of stay in all lengths of stay compared to others. The remaining six categories have relatively uniform quantities, with `MICU` having slightly more than the other five.
