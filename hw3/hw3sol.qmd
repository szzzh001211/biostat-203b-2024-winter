...
ADT_plot <- empty_plot + 
  geom_segment(data = ADT_info, aes(x = intime, xend = outtime, y = "ADT", 
               ...
               linewidth = ifelse(ADT_info$ADT_Type == "ICU/CCU", 8, 3)) +
  scale_color_manual(values = c("Emergency Department" =  "red", 
                                "Medicine" = "springgreen", 
                                "ICU/CCU" = "violet", 
                                "Neuro" = "orange", 
                                "Medicine/Cardiology" = "skyblue")) +
...
For the label of procedure, since the long title is too long, we only display the first three words of the long title. And since the shape palette can deal with a maximum of 6 discrete values in RStudio, we only display the first 6 procedures and the remaining two procedures are labeled with blank labels.
