...
print $2,$5,$7,$9}' | gzip > ./labevents_filtered.csv.gz
```
...
```
**Answer:** There are 24855909 lines in this new file. It takes 28.183 seconds to ingest `labevents_filtered.csv.gz` by using `read_csv`.
...
system.time(filtered_data_csv <- labevents_csv |> select(subject_id, itemid, charttime, valuenum) |> 
  filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) |> collect())
```

```{r,eval=TRUE}
nrow(filtered_data_csv)
head(filtered_data_csv,10)
```
...
system.time(filtered_data_parquet <- labevents_parquet |> select(subject_id, itemid, charttime, valuenum) |> 
  filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) |> collect())
```

```{r,eval=TRUE}
nrow(filtered_data_parquet)
head(filtered_data_parquet,10)
```
...
system.time(filtered_data_duckdb <- labevents_duckdb |> select(subject_id, itemid, charttime, valuenum) |> 
  filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) |> collect())
```

```{r,eval=TRUE}
nrow(filtered_data_duckdb)
head(filtered_data_duckdb,10)
```
