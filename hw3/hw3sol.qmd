...
g <- arrangeGrob(g1, g2, g3, g4, g5, ncol = 1, nrow = 5)
ggtitle(g, "Length of ICU stay vs First vital measurements within the ICU stay")
plot(g)
```
**Comment:** The distributions of `heart_rate`, `non_invasive_blood_pressure_systolic`, `non_invasive_blood_pressure_diastolic`, `respiratory_rate`, and `temperature_fahrenheit` are relatively uniform. As the values of these measurements increase, the length of stay tends to decrease.
...
plot(h)
```
**Comment:** Overall, the length of ICU stay tends to be relatively short. As the length of stay increases, the frequency decreases gradually. `Neuro Intermediate`, `Neuro Stepdown`, and `Neuro SICU` have significantly shorter lengths of stay across various durations compared to others. The distributions of the remaining six are relatively uniform, with MICU having slightly more than the others.
