...
**Answer:** The data files are distributed as `.csv.gz` files instead of `.csv` files because the `.csv.gz` files are compressed and take up less storage space. These compressed files can be transferred more quickly over networks and are easier to manage when it comes to storage and backup processes. The `.csv.gz` files are compressed using the `gzip` command. The following bash command displays the contents in the folders `hosp` and `icu`. The `hosp` folder contains all data acquired from the hospital wide electronic health record. Information covered includes patient and admission information, laboratory measurements, microbiology, medication administration, and billed diagnoses. The `icu` folder contains information collected from the clinical information system used within the ICU. Documented data includes intravenous administrations, ventilator settings, and other charted items.
```{bash}
#| eval: true
ls -l ~/mimic/hosp/
ls -l ~/mimic/icu/
...
**Answer:** The `zcat` command is used to display the contents of a compressed file without decompressing them. The `zless` command is used to display the contents of a compressed file one screen at a time. The `zmore` command is used to display the contents of a compressed file page by page. The `zgrep` command is used to search for a specified pattern in a compressed file.
...
**Answer:** The following bash script displays the first few lines of `admissions.csv.gz`. There are 431232 rows in this data file. There are 180733 unique patients in this data file. It does not match the number of patients listed in the `patients.csv.gz` file, 299713.
```{bash}
#| eval: true
...
**Answer:** The following bash script displays the possible values and the count for each unique value of the variable `admission_type`. There are 9 unique values for the variable `admission_type`. The counts for each unique value are as follows.
```{bash}
#| eval: true
...
```
**Answer:** The following bash script displays the possible values and the count for each unique value of the variable `admission_location`. There are 11 unique values for the variable `admission_location`. The counts for each unique value are as follows.
```{bash}
#| eval: true
...
```
**Answer:** The following bash script displays the possible values and the count for each unique value of the variable `insurance`. There are 3 unique values for the variable `insurance`. The counts for each unique value are as follows.
```{bash}
#| eval: true
...
```
**Answer:** The following bash script displays the possible values and the count for each unique value of the variable `ethnicity`. There are 33 unique values for the variable `ethnicity`. The counts for each unique value are as follows.
```{bash}
#| eval: true
...
du -h labevents.csv 
```
**Answer:** The following bash script compares the run times of `zcat < ~/mimic/labevents.csv.gz | wc -l` versus `wc -l labevents.csv`. The run time of `zcat < ~/mimic/labevents.csv.gz | wc -l` is 14.268s. The run time of `wc -l labevents.csv` is 15.972s. 
```{bash}
#| eval: true
time zcat < ~/mimic/hosp/labevents.csv.gz | wc -l
time wc -l labevents.csv
```
**Answer:** The trade off between storage and speed for big data files is that compressed files take up less storage space but take longer to run. Uncompressed files take up more storage space but take less time to run.
...
**Answer:**  The `date` command displays the current date and time. The `hostname` command displays the name of the current host system. The `arch` command displays the machine hardware name. The `uname -a` command displays the operating system name. The `uptime` command displays the current time, how long the system has been running, how many users are currently logged on, and the system load averages for the past 1, 5, and 15 minutes. The `who am i` command displays the current user's login name and terminal line. The `who` command displays the current users logged on. The `w` command displays the current users logged on and what they are doing. The `id` command displays the current user's user and group IDs. The `last | head` command displays the last 10 users logged on. The `echo {con,pre}{sent,fer}{s,ed}` command displays the strings "consents", "confer", "consented", "conferred", "presents", "prefer", "presented", and "preferred". The `time sleep 5` command displays the time it takes to run the command `sleep 5`, which is 5 seconds. The `history | tail` command displays the last 10 commands that were run.
```{bash}
#| eval: true
cal
```
...
hostname
```
**Answer:** The `hostname` command displays the name of the current host system.
```{bash}
#| eval: true
arch
```
**Answer:** The `arch` command displays the architectural information about the operating system.
```{bash}
#| eval: true
uname -a
```
**Answer:** The `uname -a` command displays the the name, version and other details about the current machine and the operating system running on it.
```{bash}
#| eval: true
uptime
```
...
who
```
**Answer:** The `who` command displays the login information of all users.
```{bash}
#| eval: true
w
```
...
```
**Answer:** The `echo {con,pre}{sent,fer}{s,ed}` command selects the contents in curly brackets and group them together. It displays the strings "consents", "consented", "confers", "confered", "presents", "presented", "prefers", and "prefered".
```{bash}
#| eval: true
time sleep 5
```
**Answer:** The `time sleep 5` command displays the time it takes to run the command `sleep 5`, which is 5 seconds.
```{bash}
#| eval: true
history | tail
```
