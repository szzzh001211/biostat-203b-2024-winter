...
admissions_tble |> count(subject_id) |> 
  ggplot() + 
  geom_histogram(mapping = aes(x = n), binwidth = 5, fill = "salmon", 
                 color = "black") + 
...
ggplot(admissions_length) + 
  geom_bar(mapping = aes(x = value)) + 
  labs(title = "Distribution of Length of Admission") + 
  xlab("Length") + 
  ylab("Count") + 
  theme_bw()
```
...
ggplot(mimic_icu_cohort, aes(x = age_at_intime, y = los)) +
  ...
  labs(title = "Length of ICU Stay vs Age at Intime", x = "Length of Stay") +
  theme_bw()
```
**Comment:** The length of ICU stay tends to be relatively short for all ages and the distributions are all a little left-skewed. As the age increases, the length of stay tends to increase and the length of stay seems to be longer for people aged 65-80.
...
g <- grid.arrange(g1, g2, g3, g4, g5, ncol = 2, top = "Length of ICU Stay vs 
