...
```
**Answer:** It takes around 0.034 seconds for the ingest+select+filter process. There are 24855909 rows and they match those in Q2.3. Here I use `Sys.setenv(TZ = "UTC")` to avoid the time zone issue because in RStudio the default time zone is set to the Los Angeles's time zone.
...
```
**Answer:** It takes around 0.329 seconds for the ingest+convert+select+filter process. This may be because it has more commands than the Q2.4 and Q2.5. There are 24855909 rows and they match those in Q2.3.
