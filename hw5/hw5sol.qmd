...
```
**Answer:** When `fold` = 2 and `epochs` = 30, the best model has `hidden_units` = 10 and `dropout` = 0.125. The best model has a `roc_auc` value of 0.614 and an `accuracy` of 0.582.
...
```
**Answer:** The stacked model includes `XGBoost` and `MLP` as the best model. From the weight plot, we can see the top three all come from `XGBoost` and the first one has the weight 1.487, which is almost three times the second one. The best penalty is 0.001. It is surprising that the `Logistic Regression` model is not included in the stacked model. This may be because the `Logistic Regression` model is not as good as the other two models in terms of `roc_auc` and `accuracy`. And the hyperparameters for the `Logistic Regression` model are not tuned so much and the model is not as complex as the other two models. 
